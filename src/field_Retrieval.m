...
for sampleNum = 1:length(sampleList)
    ...
    try
        ...
        for kk = 1:6
            subplot(3,3,kk)
            imagesc(squeeze(retPhase(:,:,round(nFrames/6*kk))), [-3 3])
            axis image; axis off; colorbar; colormap(jet)
            title(sprintf('frame %d', round(nFrames/6*kk)))
        end
        subplot(3,3,[7 9])
        imagesc(squeeze(retPhase(:,end/2,:)), [-3 3])
        axis tight; colorbar; colormap(jet)
        xlabel('frame'); ylabel('y (px)'); title('XT kymograph')
...
try
    fig = figure('Visible','off','Renderer','painters','Position',[0 0 900 400]);
    hold on
    for sampleNum = 1:length(fieldList)
        ...
        plot(temp, 'or', 'MarkerSize', 3)
        temp = temp - circshift(temp, 1);
        plot(temp, 'og', 'MarkerSize', 3)
    end
    ylim([-3 3]); xlabel('frame'); ylabel('mean |phase| (rad)')
    legend({'mean |phase|','frame diff'}, 'Location','northeast')
    title('Field inspection'); grid on
    exportgraphics(fig, inspectionPng, 'Resolution', 150)
    close(fig)
